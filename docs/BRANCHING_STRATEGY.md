...
This document explains how we organize branches and what naming conventions we
use.
...
- **`release/vX.Y.Z`** — prepare a release (rarely used; usually direct releases
  from `main`)
...
1. **Use lowercase** ✅ `feature/add-cover` — YES ❌ `Feature/Add-Cover` — NO

2. **Use hyphens for word separation** ✅ `feature/add-cover-position` — YES ❌
   `feature/add_cover_position` — NO

3. **Be descriptive but concise** ✅ `feature/add-cover-position-sync` — YES ❌
   `feature/stuff` or `f1` — NO

4. **No special characters except hyphen** ✅ `bugfix/fix-config-flow-timeout` —
   YES ❌ `bugfix/fix-config.flow@timeout` — NO

---

## When to Delete Your Branch

After your PR is merged, GitHub offers to delete your branch. **Do it** — keeps
the repo clean.
...
3. Open a **draft PR** early to discuss approach (GitHub UI: click "Draft"
   button).
...
- See [CONTRIBUTOR_WORKFLOW.md](CONTRIBUTOR_WORKFLOW.md) for the full
  step-by-step guide.
- Check GitHub's
  [branching docs](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/proposing-changes-to-your-work-with-pull-requests/creating-and-deleting-branches-within-your-repository)
  for more.
