...
- Push/PR run fast checks (preflight/fast checks): `black --check`,
  `isort --check-only`, smoke tests.
- Full QA Matrix runs only after adding the `run-qa-matrix` label or commenting
  `/run-qa-matrix`.
...
- Maintainers update `manifest.json`, `CHANGELOG.md`, `RELEASE_NOTES.md` before
  releases.
- Run `./scripts/release_automation.sh --dry-run` before tagging.

Troubleshooting (brief)

- `black --check` / `isort --check-only` failed: run `black .` / `isort .`
...
This guide is for **anyone** who wants to contribute to LUXORliving — you don't
need admin access or special knowledge. Just follow these steps.
...
- **Feature:** `feature/your-feature-name` (e.g.,
  `feature/add-light-brightness-sync`)
- **Bug fix:** `bugfix/your-bug-name` (e.g., `bugfix/fix-timeout-on-reboot`)
- **Documentation:** `docs/your-doc-name` (e.g.,
...
**Important:** All tests must pass before you push. If a test fails, fix it —
don't skip it.
...
Go to https://github.com/phismith91/luxorliving and click **Compare & pull
request**.
...
- **A reminder comment** is posted about branch protection (you can ignore this;
  it's for repo admins).
...
If your change is significant or if a maintainer asks, you can trigger the full
QA Matrix (Python 3.11/3.13 × Home Assistant 2025.12/2026.1/latest) by:
...
Your IDE might have different settings than the project. Trust pre-commit — it's
configured to match the CI checks.
...
- Check [BRANCHING_STRATEGY.md](BRANCHING_STRATEGY.md) for branch naming
  details.
