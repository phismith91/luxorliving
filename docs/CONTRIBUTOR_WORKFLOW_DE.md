...
- Benutze Branch‑Namen nach Konvention (siehe
...
- Öffne eine PR gegen `main` und fülle das PR‑Template aus (Beschreibung, Tests,
  Checklist).
...
- Releases nur von Maintainer: `manifest.json`, `CHANGELOG.md`,
  `RELEASE_NOTES.md` updaten.
- Vor Release: `./scripts/release_automation.sh --dry-run` ausführen.

Troubleshooting (Kurz)

- `black --check`/`isort --check-only` fehlschlägt → lokal `black .` / `isort .`
  ausführen.
