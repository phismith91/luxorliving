...
- **Zombie-tunnel backlog never drained, permanent 5-minute crash loop (#201)**: Marcus' 2026-09-08 logs show the rc.4 backpressure fix stops the outgoing queue from growing *further*, but never lets it shrink — reads keep skipping "gateway not keeping up" at 50-110 queued telegrams indefinitely, `zombie tunnel detected` fires every ~5 minutes, and each forced reconnect reports "recovery complete" while the same backlog is already back above 50 within a minute. Root cause: nothing paused new reads *before* the backlog built up, and nothing gave the tunnel a quiet window to actually drain right after a reconnect — the integration immediately resumed polling at full rate, refilling the queue as fast as it drained. Fixed: reads now pace at a minimum 150ms spacing (350ms during startup or right after a reconnect), a burst of 3+ confirmation timeouts within 60s pauses all non-essential reads for 120s, and every reconnect/zombie-recovery now gets a 45s read-quiet window followed by 5 minutes of slower "cautious" pacing. Also removes two redundant read patterns that fed the flood: lights and dimmers now read only their preferred status address instead of both status and control, and climate/cover no longer issue a fresh KNX read on every coordinator poll tick.
...
- **Teardown leaked live xknx instances that occupied every IP1 tunnel slot (#201, rc.4)**: Marcus' 2026-08-14 logs (on rc.3) showed the retry loop working — 42 attempts — but every one dying with `E_NO_MORE_CONNECTIONS`, and 8 tunnel source addresses (15.15.240-247) concurrently flooding GroupValueReads for hours, so bad that even LuxorPlay couldn't steer. Two-part root cause: (1) xknx moves outgoing telegrams from `xknx.telegrams` into an internal second queue (`telegram_queue.outgoing_queue`) which the rate limiter drains at ~1 per 3s on a zombie tunnel — the teardown drain from v1.2.1 only emptied the *first* queue, so `xknx.stop()`'s queue-join hung on the ~1600-telegram backlog (5min zombie window × 171 polled entities) and always hit the 15s stop-timeout; (2) the timeout path then "abandoned" the instance by just dropping the reference — heartbeat, auto-reconnect task, and transport kept running, each leaked instance held (and on flush re-grabbed within 3s) an IP1 tunnel slot forever and kept transmitting its backlog for ~80min. Eight recovery cycles = all 8 IP1 slots occupied by our own zombies = permanent `E_NO_MORE_CONNECTIONS`. Fixed: teardown now drains *both* queues (so `stop()` completes normally), and if `stop()` still times out the tunnel interface is force-closed via `knxip_interface.stop()` (DisconnectRequest + reconnect-task stop + transport close — the IP1 slot is actually released) instead of abandoned alive.
- **No backpressure on entity-poll reads (#201, rc.4)**: during a zombie window entity polling queued ~17× faster than the rate limiter drained, building the doomed backlog that caused the hang above. Reads are now skipped (WARNING, rate-limited) while 50+ telegrams are already queued outgoing; the next poll cycle retries once the backlog clears.
- **Log hygiene for connect failures (#201, rc.4)**: repeated identical connect failures now log the full traceback only once (42 identical `E_NO_MORE_CONNECTIONS` tracebacks drowned Marcus' 2026-08-14 log); the zombie-detection WARNING now includes the outgoing-queue depth; the retry loop logs its attempt number; dropped-telegram counts are logged at teardown.
...
- **Zombie-recovery could deadlock `_session_lock` forever**: confirmed on Marcus' 2026-07-17 log — the rc.5 zombie-tunnel watchdog fired every 5 minutes for 3h40 straight with no completion or failure ever logged, because its very first recovery attempt hung inside `async_disconnect()`/`async_setup()` (xknx has no internal timeout for a tunnel that stops acking at the protocol level) while still holding `_session_lock`. Every later recovery attempt, and the unrelated 4h periodic proactive refresh, then blocked on that same lock indefinitely — the watchdog meant to self-heal this exact failure mode instead permanently disabled all self-healing once it occurred. `_async_zombie_recover()` now bounds the disconnect+setup cycle with a 120s timeout (`ZOMBIE_RECOVERY_TIMEOUT`); on timeout the lock is released and the failure is logged so the next detection/refresh cycle can retry.
