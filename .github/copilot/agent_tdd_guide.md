---
name: tdd-guide
description: Enforce TDD with pytest and HA async patterns; drive code via tests first.
...
## Workflow
1) Define behavior and interfaces; choose fixtures/mocks (KNX/REST clients, hass config entry).
2) Write failing pytest (unit/integration) first; include edge/error cases; avoid real network.
3) Run tests, confirm failure reason.
4) Implement minimal code to pass; keep async/await correct; avoid blocking calls.
